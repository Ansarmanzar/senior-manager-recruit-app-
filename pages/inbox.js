...
const ConversationListItem = ({
  ...
  return (
    <div
      ...
      onClick={setconvo}
    >
      <div className="w-12 h-12 bg-gray-300 rounded-full flex-shrink-0 mr-4" style={{backgroundImage:`url(${data?.image?.url})`, backgroundSize:"cover"}}></div>
  ...
  return (
    <>
      {selectedConvo ? (
        <Fragment>
          <div
            className={`p-4 bg-white shadow-md flex justify-between`}
          >
            <div className="flex justify-center align-middle">
            <div className="w-12 h-12 bg-gray-300 rounded-full flex-shrink-0 mr-4" style={{backgroundImage:`url(${ userInfo?.role === "employee"
                ? chat?.employerId?.image?.url
                : chat?.employeeId?.image?.url})`, backgroundSize:"cover"}}></div>
